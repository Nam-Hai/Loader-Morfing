...
void main() {
  ...
  vec4 distxs = texture(tDist, vUv/coverRatio /1. +  0.4* vec2(cos(time) , sin(time)) + 24.1);

  vec2 largeDisplacment = vec2(cos(time * 2.), sin(time * 2.)) *mix(0.1, 1. ,progE)* (- distl.g + distl.b)  * 0.02;
  vec2 smallDisplacment = vec2(cos(time*2.), sin(time* 2.)) *progE * (dists.r + dists.g -4. * dists.b) * 0.007;
  vec2 smallDisplacmentNormal = vec2(-sin(time*2.), cos(time* 2.)) *mix(0.0, 1., progE) * (-2. *distxs.r ) * 0.002;
