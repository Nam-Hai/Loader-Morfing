...
N.Raf = class {
  ...
  loop(r) {
    ...
    let elapsed = this.clock.elapsedTime * 1000
    if (this.on) {
      for (let t = this.l(); 0 <= t; t--) {
        const e = this.arr[t];
        N.Is.def(e) && ((!N.Is.und(e.sT) && !Number.isNaN(e.sT)) || (e.sT = elapsed), s = elapsed - e.sT, e.cb({ elapsed: s, delta: d }))
      }
    }
    this.raf()
  }
